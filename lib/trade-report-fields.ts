...
export const tradeReportConfigs: TradeReportConfig[] = [
  {
    ...
    fields: [
      ...
      { id: "machines", type: "check", label: "使った重機", options: ["ユンボ", "ブレーカー", "その他"] },
...
export function getTradeReportConfig(trade: string) {
  return tradeReportConfigs.find((config) => config.trade === trade.trim());
}
